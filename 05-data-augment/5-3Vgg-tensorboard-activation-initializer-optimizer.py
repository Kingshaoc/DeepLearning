# Vgg网络的优化
# 1.activation激活函数 relu aigmoid tanh
# 2.initializer 初始化参数 he，xavier，normal
# 3.optimizer：学习率的优化方法 Adam,Momentum,Gradient Descent
...
def convent(inputs, activation, kernel_initializer):
    # 卷积核参数的初始化kernel_initializer 默认的情况下是  tf.glorot_uniform_initializer
    conv1_1 = tf.layers.conv2d(inputs, 32, (3, 3), padding='same', activation=activation,
                               kernel_initializer=kernel_initializer,
                               name='conv1_1')
    # vggnet添加新的卷积层
    conv1_2 = tf.layers.conv2d(conv1_1, 32, (3, 3), padding='same', activation=activation,
    ...
    # 第二个卷积层
    conv2_1 = tf.layers.conv2d(pooling1, 32, (3, 3), padding='same', activation=activation,
                               kernel_initializer=kernel_initializer, name='conv2_1')
    # vgg 添加新的卷积层
    conv2_2 = tf.layers.conv2d(conv2_1, 32, (3, 3), padding='same', activation=activation,
    ...
    # 第三个卷积层
    conv3_1 = tf.layers.conv2d(pooling2, 32, (3, 3), padding='same', activation=activation,
                               kernel_initializer=kernel_initializer, name='conv3_1')
    # vgg 添加新的卷积层
    conv3_2 = tf.layers.conv2d(conv3_1, 32, (3, 3), padding='same', activation=activation,
...
flatten = convent(x_image, tf.nn.relu,tf.truncated_normal_initializer)
