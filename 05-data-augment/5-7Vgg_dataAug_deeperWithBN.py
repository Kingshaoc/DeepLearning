...
def conv_wrapper(inputs,name,is_training,output_channel=32,kernel_size=(3, 3),activation=tf.nn.relu,padding='same'):
    ...
    with tf.name_scope(name):
        #先将激活函数设置为null
        conv2d = tf.layers.conv2d(inputs,output_channel,kernel_size,padding=padding,activation=None,name=name + '/conv2d')
        #batch_normalization需要维护一个均值和方差在训练过程和测试过程是不一样的
        # 训练的时候是在一个batch上计算得到的 测试的时候是在整个数据及上加权平均得到的
        bn = tf.layers.batch_normalization(conv2d,training=is_training)
        #加上batch_normalization之后再relu
...
# 78.04% -82.6%（图像增强之后）-83.4%（deeper）-85.6%（bn）
with tf.Session() as sess:
    ...
    for i in range(train_steps):
        ...
        eval_ops_result = sess.run(eval_ops, feed_dict={x: batch_data, y: batch_labels,is_training:True})
